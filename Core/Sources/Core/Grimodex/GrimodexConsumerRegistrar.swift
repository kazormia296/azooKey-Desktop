...
public enum GrimodexConsumerRegistrarError: Error, Equatable, Sendable {
    case invalidVersion
...
private struct GrimodexConsumerAtomicWriter: Sendable {
    func replace(_ data: Data, at destination: URL) throws {
        ...
        let temporaryFD = temporaryName.withCString {
            openat(directoryFD, $0, flags, mode_t(0o600))
        }
    ...
    private func syncDirectory(_ descriptor: Int32) throws {
        while fsync(descriptor) != 0 {
            ...
            #if canImport(Darwin)
            // Darwin filesystems may reject fsync for directory descriptors.
            // The file itself was synced before the atomic rename, so retain
            // that guarantee when directory syncing is unavailable.
            if errno == EINVAL {
                return
            }
            #endif
...
public final class GrimodexConsumerRegistrar: @unchecked Sendable {
    public static let consumerID = "azookey-grimodex"
    ...
    @discardableResult
    public func registerNow() throws -> URL {
        registrationLock.lock()
        defer { registrationLock.unlock() }
